...
const randomSkew = () => {
  ...
  // }
  // if (weightedSelection > 31 && weightedSelection < 100) {
...
const randomOffset = () => {
  ...
  if (weightedSelection > 70 && weightedSelection < 90) {
...
export default function OpepenOffsetA({ address }: Props) {
  const svg = (address: string) => {
    const minOpacity = 60;
    const maxOpacity = 80;
    const colors = walletColors(address, 5, 10, 40, 9, 49, 55);
    const ogColors = gradientForAddress(address, 46, 60, 5, 85, 95);
    ...
    <path fill="${ogColors[2]}" d="M2000 0H0v2000h2000V0Z" opacity="1" />
